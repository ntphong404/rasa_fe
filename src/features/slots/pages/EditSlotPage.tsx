...
export function EditSlotPage() {
  ...
  const slotData: ISlot | undefined = location.state?.slot;
  ...
  useEffect(() => {
    if (!slotData) {
      toast.error(t("slotpage-no-data-found"));
      navigate("/slots");
      return;
    }
    ...
    setDefine(slotData.define || "");
    setBotIds(slotData.botIds || []);
    setEntityId(slotData.entity || "");
    setIntentId(slotData.intent || "");
    setActionId(slotData.action || "");
    setRolesText((slotData.roles || []).join(", "));

    // Parse slot type and influence_conversation from existing define
    const existingDefine = slotData.define || "";
    const typeMatch = existingDefine.match(/type:\s*(\w+)/);
    if (typeMatch) {
      setSlotType(typeMatch[1]);
    }
    const influenceMatch = existingDefine.match(/influence_conversation:\s*(true|false)/);
    if (influenceMatch) {
      setInfluenceConversation(influenceMatch[1] === "true");
    }
    const valueMatch = existingDefine.match(/value:\s*"([^"]*)"/);
    if (valueMatch) {
      setIntentValue(valueMatch[1]);
    }
    const catMatch = existingDefine.match(/values:\s*\n((?:\s+- .+\n?)+)/);
    if (catMatch && typeMatch?.[1] === "categorical") {
      const vals = catMatch[1].split("\n").map((l: string) => l.replace(/^\s+-\s*/, "").trim()).filter(Boolean);
      setCategoricalValues(vals.join(", "));
    }
  }, [navigate, slotData, t]);
  ...
  useEffect(() => {
    ...
    const loadSelectedLabels = async () => {
      try {
        if (slotData.entity) {
          const entity = await entityService.getEntityById(slotData.entity);
          setEntityName(entity.name || slotData.entity);
        } else {
          setEntityName("");
        }

        if (slotData.intent) {
          const intent = await intentService.getIntentById(slotData.intent);
          setIntentName(intent.name || slotData.intent);
        } else {
          setIntentName("");
        }

        if (slotData.action) {
          const action = await actionService.getActionById(slotData.action);
          setActionName(action.name || slotData.action);
        } else {
          setActionName("");
        }
      } catch (error) {
        console.error("Failed to load selected slot labels:", error);
      }
    };
  ...
  useEffect(() => {
    if (!slotData || slotData.botIds?.length) return;
    ...
    if (selectedBotId && selectedBotId !== "global") {
      setBotIds([selectedBotId]);
    }
  }, [isManager, managerAssignedBotId, selectedBotId, slotData]);
  ...
  const handleSubmit = async () => {
    if (!slotData?._id) {
      toast.error(t("slotpage-no-slot-id"));
      return;
    }
    ...
    try {
      ...
      await slotService.updateSlot(slotData._id, {
        _id: slotData._id,
        name: trimmedName,
      ...
      toast.success(t("slotpage-update-success"));
      navigate("/slots");
    } catch (error) {
      console.error("Error updating slot:", error);
      toast.error(t("slotpage-update-error"));
    } finally {
  ...
  return (
    <div className="admin-page container mx-auto max-w-4xl py-6">
      <div className="mb-6 flex items-center gap-4">
        <Button variant="ghost" size="sm" onClick={() => navigate("/slots")}>
          <ArrowLeft className="mr-2 h-4 w-4" />
          {t("Back")}
        </Button>
        <h1 className="text-3xl font-bold">{t("slotpage-edit-page-title")}</h1>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-end gap-2 pt-4">
          ...
          <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-blue-600 hover:bg-blue-700">
            {isSubmitting ? t("slotpage-updating") : t("slotpage-update-submit-button")}
          </Button>
        </div>
      </div>
    </div>
