...
import { FileCode, FileText, Edit, Eye, EyeOff, HelpCircle } from "lucide-react";
...
export default function EditResponseDialog({
  ...
  useEffect(() => {
    if (response && open) {
      ...
      if (response.define) {
        const textMatch = response.define.match(/- text:\s*"([^"]*)"/);
        if (textMatch) {
          setResponseText(textMatch[1]);
        } else {
          // If can't extract, switch to expert mode
          setExpertMode(true);
        }
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert(t("Failed to update response"));
    } finally {
